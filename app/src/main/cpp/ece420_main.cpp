...
bool lab5PitchShift(float *bufferIn) {
    ...
    if (periodLen > 0) {
        ...
        while(newEpochIdx < 2*FRAME_SIZE){
            ...
            int closest_idx = findClosestInVector(epochLocations, newEpochIdx, 1, (int)(epochLocations.size()-1));
            int win_size = epochLocations[closest_idx+1]+1-epochLocations[closest_idx-1];
            // create an array for the windowed segment
            float* win_seg = new float[win_size];
            // will never select first or last epoch location so don't worry about edge cases
            for(int i=0; i<win_size; ++i){
                win_seg[i] = bufferIn[i+epochLocations[closest_idx-1]] * getHanningCoef(win_size, i);
            }
            // perform overlap add with the windowed segment
            overlapAddArray(bufferOut, win_seg, newEpochIdx-(epochLocations[closest_idx]-epochLocations[closest_idx-1]), win_size);
            free(win_seg);
